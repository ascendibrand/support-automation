...
Models in use (both set in `orchestrate.py` as `MODEL_CLASSIFY` and `MODEL_DRAFT`):
classification = `claude-haiku-4-5-20251001`; drafting = `claude-haiku-4-5-20251001`
(deliberately switched to Haiku for both layers — cheapest current model, $1/$5 per million
tokens — since drafts are always human-reviewed before sending, so draft quality risk is low.
If draft quality ever looks off in practice, the next step up is Sonnet 4.6, not Opus, given
the cost/quality curve. Confirm exact current model string in the Anthropic console before
changing anything — strings update; e.g. older dated models like `claude-opus-4-20250514`
were retired June 15, 2026).
...
- Anthropic API: per email classified, but the hard rule skips already-replied threads for
  free. Both classification and drafting run on Haiku 4.5 (cheapest current model, $1/$5 per
  million tokens), so per-email cost is low across the board. Realistically a couple dollars/
  month at moderate volume. Storage is a non-issue (40GB; kb.json is tiny; sheet data lives in
  Google, not the server).
